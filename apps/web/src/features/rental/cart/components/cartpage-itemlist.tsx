...
function groupBundleComponents(components: CartBundleComponent[]) {
	const uncategorized: CartBundleComponent[] = [];
...
function CartPageBundleItem({
	...
	const groupedComponents = groupBundleComponents(item.components);
	...
	return (
		...
		>
			...
			<div className="space-y-px bg-neutral-100 p-3">
				{groupedComponents.uncategorized.map((component) => (
				...
				{groupedComponents.categorized.map((group) => (
					<div key={group.categoryName} className="space-y-px pt-2 first:pt-0">
	...
	return (
		<div className="mt-3 border border-neutral-100 bg-neutral-50 px-4 py-3">
			...
			<div className="flex flex-wrap gap-x-6 gap-y-1.5">
				{items.map((item) => (
					<div
						key={`${item.name}-${item.quantity}-${item.notes ?? ""}`}
