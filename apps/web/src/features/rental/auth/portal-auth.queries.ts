import type { CustomerGoogleAuthStateResponseDto } from "@repo/schemas";
import {
	queryOptions,
	useMutation,
	useQuery,
...
export function useCreateGoogleCustomerState() {
	return useMutation<
		CustomerGoogleAuthStateResponseDto,
		ProblemDetailsError,
		{ tenantId: string; portalOrigin: string; redirectPath: string }
	>({
		mutationFn: (data) => createGoogleCustomerStateFn({ data }),
		onError: (error) => {
			console.error(
				`[${error.problemDetails.status}] ${error.problemDetails.detail}`,
			);
