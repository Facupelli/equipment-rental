...
type UpdateBundleMutationOptions = Omit<
	UseMutationOptions<
		void,
		ProblemDetailsError,
		{ bundleId: string; dto: UpdateBundleDto }
	>,
	"mutationFn"
>;
...
export function useUpdateBundle(options?: UpdateBundleMutationOptions) {
	return useMutation<
		void,
		ProblemDetailsError,
		{ bundleId: string; dto: UpdateBundleDto }
	>({
		...options,
		mutationFn: ({ bundleId, dto }) =>
			updateBundle({ data: { bundleId, dto } }),
		meta: {
